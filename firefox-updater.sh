...
# dependencies:
# bash >= 5.2, ctypes.sh, ln, updater, wget, shasum
...
# Kernel information.
read -r A\[Capabilities,osType\] < "/proc/sys/kernel/ostype"
read -r A\[Capabilities,osRelease\] < "/proc/sys/kernel/osrelease"

log 0 "ctypes.sh: Check GTK and libpulse library version .."

# https://github.com/taviso/ctypes.sh
# shellcheck source=/usr/local/bin/ctypes.sh
source ctypes.sh || log $? "Unable to read/execute ctypes.sh source file .."

dlopen libpulse.so.0
dlcall -n pa_version -r string pa_get_library_version

# shellcheck disable=SC2154
A[Capabilities,libPulseVersion]=${pa_version#*:}

dlopen libgtk-3.so.0
dlcall -n gtk_get_major_version -r unsigned gtk_get_major_version
dlcall -n gtk_get_minor_version -r unsigned gtk_get_minor_version
dlcall -n gtk_get_micro_version -r unsigned gtk_get_micro_version

# shellcheck disable=SC2154
printf -v A\[Capabilities,GtkVersion\] '%d.%d.%d' \
    "${gtk_get_major_version#*:}" "${gtk_get_minor_version#*:}" "${gtk_get_micro_version#*:}"

# Close all dynamic file handles.
dlclose "${DLHANDLES[@]}"
